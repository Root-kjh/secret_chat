...
public class func{
    ...
    */
     ...
     * 
     * block_count : 4
     * block_name : 64*block_count
     * block : 234
     * 
     */
    
    public void get_client_list() {
    	String[] ip= msg.toString().split("\\n");
    	for (String i : ip) {
			main.ip_s.add(i);
		}
	}
    ...
    public void add_block(String pw, String ip, String name, String id) {
    	...
    	file_name=get_file_name(get_chain());
    	...
    	try {
        	System.arraycopy(timestamp.toString().getBytes(), 0, file, 0, 4);
        	System.arraycopy(prev_hash.getBytes(), 0, file, 4, 64);
        	System.arraycopy(sha512(pw.getBytes()), 68, file, 0, 128);
        	System.arraycopy(ip.getBytes(), 196, file, 0, 4);
        	System.arraycopy(String.valueOf(file_count).getBytes(), 200, file, 0, 4);
        	System.arraycopy(name.getBytes(), 204, file, 0, 20);
        	System.arraycopy(id.getBytes(), 224, file, 0, 10);
    		fos=new FileOutputStream("src/block/"+sha256(file).toString());
    		fos.write(file);
    		fos.close();
    	}catch (Exception e) {
